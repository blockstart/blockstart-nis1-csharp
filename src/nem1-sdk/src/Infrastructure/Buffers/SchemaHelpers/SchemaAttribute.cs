...
namespace io.nem1.sdk.Infrastructure.Buffers.SchemaHelpers
{
    internal abstract class SchemaAttribute
    {
        ...
        protected byte[] FindParam(int innerObjectPosition, int position, byte[] buffer, Constants.Value Typesize)
        {
            ...
            var data = offset == 0 ? new byte[] { 0 } : buffer.Take(offset + innerObjectPosition, (int)Typesize);
        ...
        {          
            ...
            var data =  offset == 0 ? new byte[] { 0 } : buffer.Take(vecStart, vecLength);
